...
class YoloLoss(nn.Module):
    ...
    def xywh2xyxy(self, boxes):
        ...
        ### CODE ###
        # Your code here
        re = np.zeros(boxes.shape)
        for i in range(len(boxes)):
            re[i, 0] =  boxes[i, 0] / self.S - 0.5 * boxes[i, 2]
            re[i, 1] =  boxes[i, 1] / self.S - 0.5 * boxes[i, 3]
            re[i, 2] =  boxes[i, 0] / self.S + 0.5 * boxes[i, 2]
            re[i, 3] =  boxes[i, 1] / self.S + 0.5 * boxes[i, 3]
        return re
